/**
 * Sponsored trophy mint for Farcaster mini-app.
 *
 * Warpcast often rejects / mis-maps payable eth_sendTransaction ("Unknown
 * provider RPC error"). Referral claim already works via personal_sign +
 * treasury execute — same pattern here:
 *   1) User signs a fixed mint intent message
 *   2) Treasury calls publicMint (NFT → treasury EOA via _mint)
 *   3) Treasury transferFrom → user (no IERC721Receiver required)
 *
 * POST JSON: { minter, signature, metadataUri?, issuedAt }
 * Env: PTG_PAYOUT_PRIVATE_KEY (ETH for price + gas), BASE_RPC_URL optional
 */
...
function buildMintMessage(minter, metadataUri, issuedAt) {
  return (
    "Phrase To Guess — trophy mint (Base)\n" +
    "Wallet: " +
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    /* priceWei (~5e12) + headroom na gas Base */
    const minNeed = price + 100000000000000n; /* 0.0001 ETH */
    if (bal < minNeed) {
