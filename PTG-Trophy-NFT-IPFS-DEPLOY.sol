...
contract PhraseToGuessTrophyNFT is ERC165, IERC721Metadata, Ownable, ReentrancyGuard {
    using Strings for uint256;

    string private constant FIXED_TOKEN_URI =
        "ipfs://bafkreifrpdwyvpzwavxuehhac5zluzyo7grzgblqqvxzsu7k33cjfifeui";
    ...
    function fixedTokenURI() external pure returns (string memory) {
        return FIXED_TOKEN_URI;
    }
    ...
    function mintTo(address to) public onlyOwner returns (uint256) {
        ...
        nextTokenId += 1;
        _mint(to, tokenId);
        _setTokenURI(tokenId, FIXED_TOKEN_URI);
        emit Minted(to, tokenId, FIXED_TOKEN_URI);
        return tokenId;
    }

    function publicMint(string memory) external payable nonReentrant returns (uint256) {
        require(msg.value >= priceWei, "Insufficient payment");
        uint256 tokenId = nextTokenId;
        nextTokenId += 1;
        _mint(msg.sender, tokenId);
        _setTokenURI(tokenId, FIXED_TOKEN_URI);
        emit Minted(msg.sender, tokenId, FIXED_TOKEN_URI);
